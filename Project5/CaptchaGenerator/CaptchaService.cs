...
namespace CaptchaGenerator
{
    public class CaptchaService
    {
        ...
        {
            captchaCode = GenerateRandomCode(5); // Generate a 5-character random code

            using (Bitmap bmp = new Bitmap(200, 50))
            using (Graphics gfx = Graphics.FromImage(bmp))
            using (Font font = new Font("Arial", 20, FontStyle.Bold))
            {
                gfx.Clear(Color.White); // Background color

                // Draw captcha code with a random color
                gfx.DrawString(captchaCode, font, new SolidBrush(RandomColor()), 10, 10);

                // Add noise with lines
        ...
        private void DrawRandomLines(Graphics gfx, int width, int height)
        {
            ...
            for (int i = 0; i < 10; i++) // Draw 10 random lines
            {
